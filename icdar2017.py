...
def crop_area(im, polys, tags, crop_background=False, max_tries=50, flags=None):
    '''
...
def generate_rbox(im_size, polys, tags, flags=None):
    h, w = im_size
        ...
        for i in index:
            try:
                ...
                if np.random.rand() < background_ratio:
                    # crop background
                    im, text_polys, text_tags = crop_area(im, text_polys, text_tags, crop_background=True, flags=flags)
                    if text_polys.shape[0] > 0:
                    ...
                    im, text_polys, text_tags = crop_area(im, text_polys, text_tags, crop_background=False, flags=flags)
                    if text_polys.shape[0] == 0:
                        continue
                    h, w, _ = im.shape
                    ...
                    new_h, new_w, _ = im.shape
                    score_map, geo_map, training_mask = generate_rbox((new_h, new_w), text_polys, text_tags, flags)
